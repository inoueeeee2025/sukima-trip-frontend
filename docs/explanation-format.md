...
```text
	...
	64. 仮想現在地・利用可能距離・消費済み距離・移動ログの扱いを決める・仮想旅行データの持ち方を整理する
	65. 実移動距離を仮想移動距離として使うルールを決める
	66. 距離の消費タイミングと更新方法を決める
	67. Walk mode画面を作る
	68. Walk mode画面をFigmaに合わせて整える
	69. 探索開始ボタンを作る
	70. 探索中に取得・更新する移動距離データを整理する
	71. 位置情報または移動ログから距離を更新する方法を決める
	72. 探索中のリアルタイム移動距離をstateで管理する
	73. Home(map) / Walk mode にリアルタイム移動距離を反映する
	74. 探索中の状態管理を作る
	75. 探索中ダイアログをFigmaに合わせて作る
	76. 探索終了確認ダイアログを作る
	77. Spots一覧/詳細APIを繋ぐ
	78. 探索中の現在地周辺スポットを表示できるようにする
	79. スポット発見画面を作る
	80. スポット発見画面をFigmaに合わせて整える
	81. Spots一覧画面を作る
	82. Spots一覧画面のUIをFigmaに合わせて作る
	83. Spots詳細画面を作る
	84. Spots詳細画面のUIをFigmaに合わせて作る
	85. スポット未発見時の空状態UIを作る
	86. Wikipedia APIの取得方法を決める
	87. スポットとWikipedia情報の紐付け方法を決める
	88. スポット詳細でWikipediaの説明文を表示する
	89. 必要ならWikipediaの画像や外部リンクも表示する
	90. Wikipedia情報が取れない場合の表示を作る
	91. お気に入り・like・visited系APIを繋ぐ
	92. Favorite画面を作る
	93. Favorite画面をFigmaに合わせて整える
	94. like・visited・お気に入りの見せ方をFigmaに合わせて整える
	95. 発見したスポットを記録できるようにする
	96. 一覧・詳細・探索画面で状態反映が揃うようにする
	97. 位置情報や移動データ利用の権限導線を整理する
	98. 権限未許可時の案内画面を作る
	99. エラーハンドリングを全体で整理する
	100. ローディング表示を全体で整理する
	101. ローディング中UIをFigmaに合わせて整える
	102. 外部API失敗時の再試行導線を作る
	103. 型定義を整理する
	104. APIモジュールを機能ごとに分ける
	105. デザインと共通UIを整える
	106. 共通UIコンポーネントをFigmaベースで作り直す
	107. 色・余白・文字サイズ・カードルールをFigma基準で変数化する
	108. タブバーや共通ヘッダーをFigmaに合わせて整える
	109. 画面遷移を本番寄りに整える
	110. 画面遷移と導線をFigmaのフローに合わせて調整する
	111. splash画面を作る
	112. splashから認証状態に応じて遷移する流れを整える
	113. ログイン → Home(map) → Dashboard → Home(map) の流れを確認する
	114. Home(map) → Walk mode → スポット発見 → 詳細閲覧 の流れを確認する
	115. 探索終了後の戻り先と再開導線を整える
	116. 動作確認と不具合修正をする
	117. 全画面をFigmaと見比べて差分修正する
