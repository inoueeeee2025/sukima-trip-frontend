...
```text
	...
	58. Explore mode で地図上の地点を選択し、その地点からStreetViewを開始できるようにする
	59.  Home(map)1 を訪問履歴確認用のGoogle Mapに置き換える
	60. Home(map)1とexplore modeの初期表示に関して世界の中心とズームを変える
	61.  訪問済みスポットを地図上に表示できるようにする
	62. Street Viewが表示できない原因を検知し、選択地点の近くにあるStreet Viewへ補正する
...
- 「意味」は短すぎる抽象説明で終わらせず、「どのコードをどう変えたか」「その結果どのstate / props / 分岐 / コンポーネント責務がどう変わるか」がわかる文にする。
- 「意味」は、変更前と変更後で何が変化したのかを一文で言えるように意識する。
- 型を変えた時は「何を受け取る関数 / props / state に変わったか」まで書く。
- 分岐を変えた時は「どの条件で何を描画する状態になったか」まで書く。
- コンポーネントを分けた時は「責務がどう分離されたか」まで書く。
